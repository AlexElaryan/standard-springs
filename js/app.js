(() => {
    ...
    let blogItemsSlider = null;
    const initBlogItemsSlider = () => {
        if (!blogItemsSlider) if (document.querySelector(".blog__items")) new core(".blog__items", {
            modules: [ Keyboard ],
            slidesPerView: 2.2,
            spaceBetween: 32,
            breakpoints: {
    ...
    const destroyblogItemsSlider = () => {
        if (blogItemsSlider) {
            blogItemsSlider.destroy();
            blogItemsSlider = null;
        }
    };
    ...
    window.addEventListener("resize", (function() {
        if (window.innerWidth < 1023.98) initBlogItemsSlider(); else destroyblogItemsSlider();
    }));
